...
def paireSelecteur(iqx_chromosome, SumOfIndividualList, iqx_targetRuns):
  tempuLisu = []
  ekdomChoto1 = 9999 #target run er theke amader hate thaka run gula minus kore jei value pabo.
  # sei value er ekdom choto value ta best fit, store korbo sei value, setai amader 1st parent

  ekdomChoto2 = 9999 #ekdom choto 2nd lowest value ta 2nd paren hisabe store korbo, 

  ekdomChotoIdx1 = 9999 #ekdom choto value er index

  ekdomChotoIdx2 = 9999  #ekdom choto 2nd lowest valu er index


  for itor in SumOfIndividualList:
    tempuLisu.append(abs(iqx_targetRuns - itor)) #absolute value nicchi. Target Run er sobcheye kacher value ta ber korbo evabe
  
  # prothom parent choose kortesi
  for itor in range(len(tempuLisu)): 
    if tempuLisu[itor] < ekdomChoto1:
      ekdomChoto1 = tempuLisu[itor] #ekdom minimum absolute value ta store korlam 
      ekdomChotoIdx1 = itor   #ekdom minimum absolute value er index store korlam
  
  tempuLisu[ekdomChotoIdx1] = 9999 #prothom paren(Lowest value) ber kora sesh, tai sei value ta baray dicchi, 
  #2nd parent ber korar somoy tahole r ei parent lowest value hisabe appear hobe na. 

  # second parent choose kortesi
  for itor in range(len(tempuLisu)): 
    if tempuLisu[itor] < ekdomChoto2:
      ekdomChoto2 = tempuLisu[itor] 
      ekdomChotoIdx2 = itor   
  
  tempuLisu[ekdomChotoIdx2] = 9999
  
  #tempuLisu list abar ager moto kore feltesi
  tempuLisu[ekdomChotoIdx1] = ekdomChoto1
  tempuLisu[ekdomChotoIdx2] = ekdomChoto2


  return iqx_chromosome[ekdomChotoIdx1], iqx_chromosome[ekdomChotoIdx2]
